...
export const proxy = (request: NextRequest) => {
    ...
    if (pathLocale) {
        ...
        response = locale === defaultLocale ? NextResponse.rewrite(new URL(url, request.url)) : NextResponse.redirect(new URL(url, request.url));
        nextLocale = locale;
    }
...
export const config = {
    matcher: '/((?!api|_next/static|_next/image|img/|favicon.ico|manifest.json|static/).*)',
};
